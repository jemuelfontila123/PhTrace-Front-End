...
const List = ({data, index, handleDelete, isChecked, setIsChecked, open, setOpen, setNumber, numbers, setNumbers}) => {
    const [isUser, setIsUser] = useState(false)
    let {firstName, lastName, timeStamp, temperature, status, id, email, contactNumber,role} = data;
    ...
    const handleCheck = (i,contactNumber) => {
        const newChecked = isChecked.map((element,index) => index===i ? !element: element)
        if(isChecked[i]){
    ...
    <tr>
        <td>
            <input type='checkbox' name='rows' onChange={() => handleCheck(index, contactNumber)} checked={isChecked[index]}/>
        </td>  
        <td>{index+1}</td>
