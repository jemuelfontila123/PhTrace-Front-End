...
const AdminVisitors = ({handleSuccess, handleResetMessage, handleSmsMessage}) => {
    const visitorsData = ['#','Name','Email','Contact Number','Time Visited','Temperature','Status','Actions']
    const userContext = useContext(UserContext);
    const {user} = userContext;
    const [open, setOpen] = useState(false)
    const [smsOpen, setSmsOpen] = useState(false)
    const handleClose = () => setOpen(!open)
    const [isChecked, setIsChecked] = useState(new Array(userContext.user.visitors.length).fill(false));
    ...
    const handleVisitors = () => {
        const results = user.visitors.filter(visitor => format(parseISO(visitor.timeStamp),"yyyy-MM-dd") == dateToday) 
        setVisitors(results)
    }
    const isOneChecked = () => {
    ...
    return(
        ...
        />
        <DialogForm open={open} setOpen={setOpen} title={'Filtering Visitors'} content={'Fill in the information based on what you want to filter'} onExit={() => setIsNumbers(false)}>
            <FilterVisitor 
