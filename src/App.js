...
const App  = () => {
  ...
  const setSuccessful = () => {
    ...
    setState({...state, open:true, message:'Account successfully created. You may login now'})
  }
  const run = async id => {
    ...
    <div>
        <Switch>
            <Route exact path='/'>
              ...
              : userContext.user.role[0]==='user'? <Redirect to ='/settings'/> : 
                userContext.user.role[0]==='admin' ? <Redirect to ='/dashboard'/> : null
              }   
            </Route>
            <Route path ='/scan'>
              {!userContext.user ? null:
                userContext.user.role[0]==='admin' || userContext.user.role[0]==='employee' ? <Scanner/>
                : <Redirect to='/'/> 
              }
            </Route>
            <Route path='/register'>
            ...
            <Route path='/login'>
                {userContext.user ? <Redirect to ='/dashboard'/> : 
                <>
            ...
            <Route path='/settings'>
              { !userContext.user ? 
                <Redirect to='/'/>: 
                userContext.user.role[0]==='user' ? <UserCard/>: <Redirect to='/'/>}
            </Route>
        ...
        <Snackbar autoHideDuration={3000} anchorOrigin={{ vertical, horizontal }} open={open} onClose={handleClose}>
