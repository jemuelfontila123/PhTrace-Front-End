...
const update = async(firstName, lastName, email, password) => {
    const response = await axios.post(`${baseUrl}update`,{firstName, lastName, email, password}, config)
    return response;
}
const verifyCode = async(user, code) => {
    const response= await axios.post(`${baseUrl}verify`,{id: user.id, code: code}, config)
    return response;
}
const healthCheck = async(user, checked) => {
    const response= await axios.post(`${baseUrl}healthcheck`,{id: user.id, checked}, config)
}
